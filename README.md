...
| :---------------- | :------------------------------------------------ | :--------------------------------------------------- | :------------------------ |
| **01 分析** | [step1_analyze.py](src_english/step1_analyze.py)     | 按语句语义切分长文并提取素材搜索词   | LLM 语义分析         |
| **02 配音** | [step2_tts.py](src_english/step2_tts.py)             | 驱动语音大模型生成音频（支持并发显存锁） | **VibeVoice** 推理 |
| **03 素材** | [step3_video.py](src_english/step3_video.py)         | 自动化素材抓取，支持 API 分阶段限流降级  | Pexels/Pixabay API   |
| **04 字幕** | [step5_subtitle.py](src_english/step5_subtitle.py)   | 调用 WhisperX 强制对齐，带比例计算保底   | WhisperX / 比例算法  |
| **05 翻译** | [step6_translate.py](src_english/step6_translate.py) | LLM 深度翻译，注入 ASS `\N` 强制折行     | LLM 翻译 + libass    |
| **06 混剪** | [step7_merge.py](src_english/step7_merge.py)         | 执行精细 FFmpeg 滤镜链，实现毫秒级压制   | **FFmpeg** 底层滤镜 |
| **07 封面** | [step8_cover.py](src_english/step8_cover.py)         | 提取文稿权重自动生成多尺寸 AI 艺术图     | SiliconCloud (KOLORS)|
| **08 发布** | [step9_upload.py](src_english/step9_upload.py)       | 自动绕过 WAF 校验，完成全协议栈发布     | bilibili-api 协议    |
