...
本仓库不仅是一个自动化视频生产工具，它更是我在备考期间，利用有限时间与硬件资源，探索**“高通量数据清洗 -> 异构模型调度 -> 全链路自动化生产”**系统能力的工程实践。
...
这个项目的产出不是一次简单的代码编写，也并不是我的目的。而是我在过去一年中，在平衡考研备考、探索经济独立以及追求物理与 AI 交叉研究过程中的一个阶段性记录。
...
* **半自动化阶段**：当时采用 ComfyUI（flux生图）+ GPT-SoVITS（ai声音克隆）的架构。流程涉及“原文提取 ->手动发给 Gemini 生成文稿和脚本 -> Python 素材生成 -> 人声降噪/混响处理，字幕修正 -> PR 辅助剪辑”。
...
初试结束后，我曾计划利用一个月时间通过 **Playwright**（自动化控制）+ **Remotion**（前端视频框架）+ **Manim**（物理动画引擎）构建一套具备理科展现力的智能 Agent，并希望一次为敲门砖在出初试成绩前就联系您。
